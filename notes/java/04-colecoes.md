...
Entender como o `HashMap` guarda e busca as entradas por dentro (buckets, hash, treeification, resize) é assunto clássico de entrevista e ajuda a escrever `hashCode()` e `equals()` decentes. Isso está detalhado em [HashMap por dentro](/labs/java/java/13-hashmap-por-dentro/).
